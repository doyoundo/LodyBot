...
class Song():
    ...
    class Songinfo:
        ...
        def format_output(self, play_type):
            embed = discord.Embed(title=play_type, description="[{}]({})".format(
                self.title, self.webpage_url), color=config.EMBED_COLOR)
            ...
            if self.duration is not None:
                embed.add_field(name=config.SONGINFO_DURATION, value="{}".format(
                    str(datetime.timedelta(secpmds=self.duration))), inline=False)
            else:
